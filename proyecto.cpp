...
public:
    Productor(int id, int producciones) : id(id), numero_producciones(producciones) {
        archivo.open("documento.txt", ios::app); 
    }
    ...
    void producir() {
        for (int i = 1; i <= numero_producciones; ++i) {
            ...
            if (sem_trywait(&empty_slots) != 0) {
                ...
                archivo << "Productor " << id << " espera, buffer lleno." << endl;
                archivo.flush();
...
int main(int argc, char *argv[]) {
    ...
    pthread_mutex_init(&archivo_mutex, nullptr);


    vector<Productor> productores; 
    for (int i = 0; i < NP; ++i) {
        productores.emplace_back(i + 1, NPP); 
    }

    vector<thread> threads_productores;
    for (auto &productor : productores) {
        threads_productores.push_back(thread(&Productor::producir, &productor)); 
    }

    vector<Consumidor> consumidores; 
    for (int i = 0; i < NC; ++i) {
        consumidores.emplace_back(i + 1, NCC); 
    }

    vector<thread> threads_consumidores;
    for (auto &consumidor : consumidores) {
        threads_consumidores.push_back(thread(&Consumidor::consumir, &consumidor));
    }

    for (auto &productor_thread : threads_productores) {
        productor_thread.join();
    }
    for (auto &consumidor_thread : threads_consumidores) {
